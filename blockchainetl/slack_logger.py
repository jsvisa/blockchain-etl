# fork from https://github.com/junhwi/python-slack-logger/blob/master/slack_logger.py
import json
import logging
...
class SlackHandler(HTTPHandler):
    ...
    def mapLogRecord(self, record):
        ...
        if isinstance(self.formatter, SlackFormatter) or isinstance(
            self.formatter, SlackCronFormatter
        ):
...
class SlackFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord):
        ...
        ret["text"] = "\nFilepath: `{}:{}`\n Msg: {}".format(
            record.pathname,
            record.lineno,
            super(SlackFormatter, self).format(record),
        )
        return ret


class SlackCronFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord):
        ret = {}
        if record.levelname == "INFO":
            ret["color"] = "good"

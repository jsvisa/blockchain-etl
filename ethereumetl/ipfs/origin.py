...
def _get_origin_shop_products(log: EthLog, listing_id, ipfs_client, shop_ipfs_hash):
    results = []
    shop_index_page = ipfs_client.get(shop_ipfs_hash + "/index.html")
    shop_data_dir = _get_shop_data_dir(shop_index_page)

    path = (
        "{}/{}".format(shop_ipfs_hash, shop_data_dir)
        if shop_data_dir
        else shop_ipfs_hash
    )
    ...
    try:
        ...
        logger.error(
            "Listing {} Failed downloading product {}: {}".format(
                listing_id, products_path, e
            )
        )
        return results
    ...
    for product in products:
        ...
        if len(variants) > 0:
            ...
            for variant in variants:
                ...
                result.ipfs_path = product_base_path
                result.external_id = (
                    str(variant.get("externalId"))
                    if variant.get("externalId")
                    else None
                )
                result.parent_external_id = (
                    str(product.get("externalId"))
                    if product.get("externalId")
                    else None
                )
...
def get_origin_marketplace_data(log: EthLog, listing_id, ipfs_client, ipfs_hash):
    # Load the listing's metadata from IPFS.
    try:
        listing_data = ipfs_client.get_json(ipfs_hash)
    except Exception as e:
        logger.error(
            "Extraction failed. Listing {} Listing hash {} - {}".format(
                listing_id, ipfs_hash, e
            )
        )
        return None, []
    ...
    listing.block_number = log.block_number
    listing.log_index = log.log_index
    ...
    if shop_ipfs_hash:
        try:
            shop_listings = _get_origin_shop_products(
                log, listing_id, ipfs_client, shop_ipfs_hash
            )
        except Exception as e:
            logger.error(

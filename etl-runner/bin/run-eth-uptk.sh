...
exec pipenv run python ./hack/upsert-token.py \
    --chain=ethereum \
    --db-data-url="$BLOCKCHAIN_ETL_TSDB_URL" \
    --db-meta-url="$BLOCKCHAIN_ETL_TSDB_URL" \
    --last-synced-block-file=.priv/ethereum/upsert-token.txt
